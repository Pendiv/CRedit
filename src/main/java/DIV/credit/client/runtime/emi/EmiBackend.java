...
public final class EmiBackend implements CreditRuntimeBackend {
    ...
    private static IngredientSpec toSpec(EmiStack stack) {
        ...
        if (stack instanceof FluidEmiStack fls) {
            ...
            // 1.21: FluidStack NBT は DataComponents 化、 EMI FluidEmiStack の hasNbt/getNbt 廃止。
    ...
    public List<CreditRecipe> getRecipes(CreditCategory category) {
        ...
        try {
            ...
            for (var emiRecipe : EmiApi.getRecipeManager().getRecipes(emiCat)) {
                ...
                try {
                    // 1.21: getBackingRecipe() は RecipeHolder<?> を返しうる → unwrap
                    Object br = emiRecipe.getBackingRecipe();
                    if (br instanceof net.minecraft.world.item.crafting.RecipeHolder<?> rh) backing = rh.value();
                    else if (br instanceof net.minecraft.world.item.crafting.Recipe<?> rr) backing = rr;
