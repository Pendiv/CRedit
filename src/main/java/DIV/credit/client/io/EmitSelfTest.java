...
public final class EmitSelfTest {
    ...
    public static VerifyResult tryVerifyFields(String typeId, LinkedHashMap<String, String> fields, String recipeId) {
        try {
            ...
            try { typeRl = ResourceLocation.parse(typeId); }
            ...
            // 1.21: RecipeSerializer.fromJson 廃止 → codec().codec().parse(ops, json) で schema 検証。
            // ItemStack/Ingredient codec は registry 参照するため、world 接続中は RegistryOps を使う。
            var parsed = ser.codec().codec().parse(jsonOps(), json);
            if (parsed.error().isPresent()) {
                return VerifyResult.fail(parsed.error().get().message());
            }
    ...
    private static DynamicOps<JsonElement> jsonOps() {
        try {
            ...
            if (conn != null) return RegistryOps.create(JsonOps.INSTANCE, conn.registryAccess());
