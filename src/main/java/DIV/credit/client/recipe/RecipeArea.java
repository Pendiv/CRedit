...
public class RecipeArea {
        ...
        try {
            ...
            for (var v : views) {
                ...
                if (ing instanceof net.minecraft.world.item.ItemStack stack
                    && !stack.isEmpty() && !stack.getComponentsPatch().isEmpty()) {
                    return true;
                }
            }
    ...
    private static <T> IRecipeLayoutDrawable<?> createDrawableFromDraft(IRecipeManager rm,
        ...
        try {
            return rm.createRecipeLayoutDrawable(cat, (T) raw, empty).orElse(null);
        } catch (ClassCastException cce) {
            // JEI 1.21: vanilla category は RecipeType<RecipeHolder<T>> のため raw Recipe を RecipeHolder で包む。
            //   Mek/GT 等 (= 非 RecipeHolder) は raw 経路で成功するので、 ここは ClassCast 時のみ retry。
            return wrapInHolderAndBuild(rm, cat, raw, empty);
        }
    }

    @SuppressWarnings("unchecked")
    private static <T> IRecipeLayoutDrawable<?> wrapInHolderAndBuild(IRecipeManager rm,
                                                                     IRecipeCategory<T> cat,
                                                                     Object raw, IFocusGroup empty) {
        if (raw instanceof net.minecraft.world.item.crafting.Recipe<?> rcp) {
            var holder = new net.minecraft.world.item.crafting.RecipeHolder<>(
                net.minecraft.resources.ResourceLocation.fromNamespaceAndPath("credit", "preview"), rcp);
            return rm.createRecipeLayoutDrawable(cat, (T) (Object) holder, empty).orElse(null);
        }
        return null;
    }
    ...
    private void updateSlotDisplays() {
        ...
        for (int i = 0; i < views.size(); i++) {
            ...
            // JEI 1.21: RecipeSlot.displayIngredients は List<@Nullable ITypedIngredient<?>> (Optional 包装は廃止)
    ...
    private static void renderSpec(GuiGraphics g, IJeiRuntime rt, IngredientSpec spec, int x, int y) {
        try {
            ...
            if (spec instanceof IngredientSpec.Fluid fl && !fl.stack().isEmpty()) {
                rt.getIngredientManager().getIngredientRenderer(NeoForgeTypes.FLUID_STACK)
    ...
    private boolean tryIncrementCircuit(int slotIndex, IngredientSpec current) {
        ...
        // 1.21: getOrCreateTag 廃止 → CUSTOM_DATA component
        net.minecraft.world.item.component.CustomData cd0 =
            copy.getOrDefault(net.minecraft.core.component.DataComponents.CUSTOM_DATA,
                net.minecraft.world.item.component.CustomData.EMPTY);
        net.minecraft.nbt.CompoundTag tag = cd0.copyTag();
