...
public interface RecipeDraft {
    ...
    /**
     * v2.1.5 / 1.21-port: ItemStack を KubeJS ingredient 文字列に変換。DataComponents 対応。
     * <p>1.20.1 の NBT + {@code strongNBT()} は 1.21 で消滅。代わりに vanilla ItemParser 文法
     * {@code 'ns:path[component=value,...]'} (= /give と同じ構文、KubeJS が解釈) を組む。
     * <ul>
     *   <li>component 無し, count=1:  {@code 'ns:path'}</li>
     *   <li>component 無し, count&gt;1: {@code Item.of('ns:path', count)}</li>
     *   <li>component 持ち:           {@code Item.of('ns:path[comp=snbt,...]'[, count])}</li>
     * </ul>
     * component の SNBT は各 {@link DataComponentType#codec()} 経由で生成 (自前 SNBT 組みより堅い)。
     */
    static String formatItemStringWithNbt(ItemStack stack, int count) {
        ResourceLocation rl = BuiltInRegistries.ITEM.getKey(stack.getItem());
        String comps = componentsSuffix(stack);
        if (comps.isEmpty()) {
            if (count > 1) {
        ...
        // 'ns:path[...]' を single-quoted JS 文字列に: backslash / single quote をエスケープ
        String escaped = (rl + comps).replace("\\", "\\\\").replace("'", "\\'");
        if (count > 1) {
            return "Item.of('" + escaped + "', " + count + ")";
        }
        return "Item.of('" + escaped + "')";
    }

    /**
     * stack の「非デフォルト component」を vanilla ItemParser 文法の {@code [type=snbt,...]} に変換。
     * 非デフォルトが無ければ空文字。component value は各 component の persistent codec で SNBT 化する。
     * <p>registry 参照を要する codec (enchantments / potion_contents 等) のため RegistryOps を使う。
     * client が world 接続中 (= レシピ編集中) は registryAccess が取れる。
     */
    static String componentsSuffix(ItemStack stack) {
        DataComponentPatch patch = stack.getComponentsPatch();
        if (patch.isEmpty()) return "";
        HolderLookup.Provider regs = registryAccess();
        DynamicOps<Tag> ops = regs != null
            ? RegistryOps.create(NbtOps.INSTANCE, regs)
            : NbtOps.INSTANCE;
        StringBuilder sb = new StringBuilder();
        for (Map.Entry<DataComponentType<?>, Optional<?>> e : patch.entrySet()) {
            ResourceLocation typeId = BuiltInRegistries.DATA_COMPONENT_TYPE.getKey(e.getKey());
            if (typeId == null) continue;
            // minecraft: namespace は省略 (ItemParser が補完。ATM10 実例準拠)
            String name = "minecraft".equals(typeId.getNamespace()) ? typeId.getPath() : typeId.toString();
            Optional<?> value = e.getValue();
            if (value.isEmpty()) {
                // removed component (default を明示的に消した) → ItemParser 文法では "!type"
                appendComponent(sb, "!" + name);
                continue;
            }
            Codec<?> codec = e.getKey().codec();
            if (codec == null) continue;  // 非永続 component (network 専用) は文字列化不能 → skip
            Tag tag = encodeComponent(codec, ops, value.get());
            if (tag == null) continue;
            appendComponent(sb, name + "=" + tag);
        }
        return sb.length() == 0 ? "" : "[" + sb + "]";
    }

    private static void appendComponent(StringBuilder sb, String part) {
        if (sb.length() > 0) sb.append(',');
        sb.append(part);
    }

    @SuppressWarnings("unchecked")
    private static <T> Tag encodeComponent(Codec<T> codec, DynamicOps<Tag> ops, Object value) {
        try {
            return codec.encodeStart(ops, (T) value).result().orElse(null);
        } catch (Exception e) {
            return null;  // codec が値を吐けない (component 型ミスマッチ等) → skip
        }
    }

    /** client が world 接続中なら registry access (= HolderLookup.Provider) を返す。未接続なら null。 */
    @Nullable
