...
public class CookingDraft implements RecipeDraft {
    ...
    public boolean loadFromRecipe(IRecipeLayoutDrawable<?> layout) {
        ...
        // 1.21: vanilla recipe は RecipeHolder でラップされて返る → unwrap (JEI_DIFF §1)
    ...
    public Recipe<?> toRecipeInstance() {
        ...
        // 1.21: cooking recipe ctor から id 引数が消滅 (RecipeHolder へ分離)。group="" のみ。
        return switch (type) {
            case SMELTING -> new SmeltingRecipe("", type.cookCategory, ing, RecipeDraft.toOutputStack(slots[IDX_OUTPUT]), xpV, ctV);
            case BLASTING -> new BlastingRecipe("", type.cookCategory, ing, RecipeDraft.toOutputStack(slots[IDX_OUTPUT]), xpV, ctV);
            case SMOKING  -> new SmokingRecipe("",  type.cookCategory, ing, RecipeDraft.toOutputStack(slots[IDX_OUTPUT]), xpV, ctV);
            case CAMPFIRE -> new CampfireCookingRecipe("", type.cookCategory, ing, RecipeDraft.toOutputStack(slots[IDX_OUTPUT]), xpV, ctV);
        };
    }
