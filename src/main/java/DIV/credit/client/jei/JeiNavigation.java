...
public final class JeiNavigation {
    ...
    public static boolean openRecipeId(ResourceLocation recipeId,
        ...
        if (mc.level != null) {
            // 1. 元 ID で lookup
            Optional<? extends net.minecraft.world.item.crafting.RecipeHolder<?>> opt = mc.level.getRecipeManager().byKey(recipeId);
            if (opt.isPresent()) { recipe = opt.get().value(); foundId = recipeId; }
            ...
            if (recipe == null && jeiCategoryUid != null) {
                ...
                if (variant != null) {
                    Optional<? extends net.minecraft.world.item.crafting.RecipeHolder<?>> opt2 = mc.level.getRecipeManager().byKey(variant);
                    if (opt2.isPresent()) {
                        recipe = opt2.get().value();
                        foundId = variant;
    ...
    private static ResourceLocation buildPrefixVariant(ResourceLocation recipeId, String jeiCategoryUid) {
        try {
            ResourceLocation catRl = ResourceLocation.parse(jeiCategoryUid);
            ...
            return ResourceLocation.fromNamespaceAndPath(recipeId.getNamespace(), catPath + "/" + recipeId.getPath());
        } catch (Exception e) {
            return null;
        }
    }
        ...
        try {
            ...
            var matches = allInCat.stream()
                .filter(r -> {
                    ...
                    // (3) 1.21: Recipe.getId() 廃止のため省略 (= (2) registry name で代替)
                    return false;
                })
                .toList();
            if (matches.isEmpty()) {
