...
public final class RecipeReEmitter {
    ...
    static IngredientSpec parseItemSpec(String token) {
        ...
        if (token.startsWith("Item.of(")) {
            ...
            if (nbtStr != null) {
                // 1.21: ItemStack.setTag 廃止 (NBT→DataComponents)。旧 strongNBT 形式 NBT の再パースは未対応。
                //   credit 1.21 は 'id[component=value]' で emit するので通常この分岐には来ない。
                Credit.LOGGER.debug("[CraftPattern] re-emit: legacy NBT string skipped (1.21 component format): {}", nbtStr);
            }
