...
public final class ImportedRecipeRecovery {
    ...
    public static Optional<Recipe<?>> recover(ImportedRecipe r) {
        ...
        try { typeRl = new ResourceLocation(json.get("type").getAsString()); }
        catch (Exception e) {
        ...
        try {
            ...
            try { recipeRl = new ResourceLocation(r.recipeId()); }
            catch (Exception e) {
                Credit.LOGGER.info("[CraftPattern] recover: recipeId 不正 {}: {}", r.recipeId(), e.getMessage());
                return Optional.empty();
            }
            Recipe<?> recipe = ser.fromJson(recipeRl, json);
            if (recipe == null) {
    ...
    private static Recipe<?> lookupExistingRecipe(String id) {
        try {
            ...
            return mc.level.getRecipeManager().byKey(rl).orElse(null);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    private static Recipe<?> buildVanillaCookingRecipe(ImportedRecipe r) {
        ...
        return switch (method) {
            case "smelting"        -> new SmeltingRecipe(id, "", cat, ing, outStack, xp, cookingTime);
            case "blasting"        -> new BlastingRecipe(id, "", cat, ing, outStack, xp, cookingTime);
            case "smoking"         -> new SmokingRecipe(id,  "", cat, ing, outStack, xp, cookingTime);
            case "campfireCooking" -> new CampfireCookingRecipe(id, "", cat, ing, outStack, xp, cookingTime);
            default -> null;
        };
    }
    ...
    private static Recipe<?> buildVanillaStonecuttingRecipe(ImportedRecipe r) {
        ...
        ResourceLocation id = parseRecipeRl(r.recipeId());
        if (id == null) return null;
        return new StonecutterRecipe(id, "", ing, outStack);
    }
