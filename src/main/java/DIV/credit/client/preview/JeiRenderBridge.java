...
public final class JeiRenderBridge {
    ...
    private static <T> IRecipeLayoutDrawable<T> buildTyped(IJeiRuntime runtime,
        ...
        try {
            ...
            // JEI 1.21: vanilla category は RecipeType<RecipeHolder<T>> のため raw Recipe を包む
            if (recipe instanceof net.minecraft.world.item.crafting.Recipe<?> rcp) {
                ...
                @SuppressWarnings("unchecked")
                T typedHolder = (T) (Object) holder;
                return runtime.getRecipeManager()
                              .createRecipeLayoutDrawable(cat, typedHolder, empty)
                              .orElse(null);
            }
            throw cce;
        }
